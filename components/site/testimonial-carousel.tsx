      ...
      >
        {testimonials.map((item) => (
          <article key={item.name} className="w-full shrink-0 px-1">
            <div className="soft-red-panel rounded-2xl border border-red-100 p-6">
              ...
              <p className="mt-4 text-sm font-bold text-[#e10600]">{item.name}</p>
            </div>
          </article>
        ))}
      </div>
        ...
        {testimonials.map((item, dotIndex) => (
          <button
            ...
            className={`h-2.5 rounded-full transition-all ${
              dotIndex === index ? "w-8 bg-[#e10600]" : "w-2.5 bg-red-200"
            }`}
